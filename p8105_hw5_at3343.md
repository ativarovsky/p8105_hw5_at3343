...
ls_fit = lm(y ~ x_i1, data = sim_data) %>% broom::tidy() %>% filter(term == "x_i1") %>% select("estimate", "p.value")
...
Now running the simulation for different beta\_1 values.

``` r
sim_results_2 = 
  tibble(beta_1 = c(1:5)) %>% 
  mutate(
    output_lists = map(.x = beta_1, ~rerun(100, sim_slr(n = .x))),
